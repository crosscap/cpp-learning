...
public:
	...
	StrBlob(const vector<string> &vs) : data(make_shared<vector<string>>(vs)) {}
	StrBlob(StrBlob &orig) : data(make_shared<vector<string>>(orig.data)) {}

	StrBlob &operator=(const StrBlob &rhs) { data = make_shared<vector<string>>(rhs.data); }
