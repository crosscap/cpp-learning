...
int main()
{
	...
	string re_tries_wrong[4] = {
		"Oops! Nice guess but not quite it.\n",
		"Hmm. Sorry. Wrong a second time.\n",
		"Ah, this is hader than it looks, isn't it?\n",
		"It must be getting pretty frustarting by now!\n",
	};
	...
	while (next_seq == true) {
		...
		while ((got_it == false) && (go_for_it == true)) {
			...
			if (usr_guess == next_elem) {
				...
				if (num_tries < 3) {
					cout << re_tries_wrong[num_tries - 1];
				} else {
					cout << re_tries_wrong[3];
				}
